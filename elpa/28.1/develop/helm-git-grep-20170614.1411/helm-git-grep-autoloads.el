...
Show result of `git ls-files' to check files limited by pathspec which is defined by `helm-git-grep-pathspecs'." t nil)

(autoload 'helm-git-grep "helm-git-grep" "\
Helm git grep.

`helm-git-grep-sources' is used as helm sources." t nil)
...
Invoke `helm-git-grep' from isearch." t nil)

(autoload 'helm-git-grep-from-helm "helm-git-grep" "\
Invoke `helm-git-grep' from other helm." t nil)
